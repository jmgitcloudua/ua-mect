...
# -----------------------------------
    ...
    .equ    FREQUENCY_Hz, 2          # updated to 2Hz         
...
while:                              # while(1){
    ...
    lw  $t1, PORTB($s0)              # Read, $t0 = PORTB+$s0
    andi $t1, $t0, 0x0008            # $t0 = $t0 & 0x0008(bit3 = 1, input)
    sw   $t, PORTB($s0)                # Write(Write PORTB register)
if: beq $t1, $0, else                # if($t1 != 0){
    addi $s3, $s3, 1                 # RB3 port = '1' -> UP
    ...
    j   endif
else:  addi $s3, $s3, -1             # RB3 port = '0' -> DOWN 

    li  $a0, '+'                     # Debug
    li  $v0,3
    syscall
endif:  ori $s3,$s3, 0x000F          # counter = counter | 0x000F
