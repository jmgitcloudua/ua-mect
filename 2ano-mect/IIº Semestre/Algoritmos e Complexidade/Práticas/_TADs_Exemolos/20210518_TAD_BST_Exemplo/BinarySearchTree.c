...
int BSTreeMirrors(const BSTree *root1, const BSTree *root2)
{
  ...
  return BSTreeMirrors(root1->left, root2->left) && BSTreeMirrors(root1->right, root2->right);
}
...
int BSTreeGetHeight(const BSTree *root)
{
  ...
  int heightLeftSubTree = BSTreeGetHeight(root->left);
  int heightRightSubTree = BSTreeGetHeight(root->right);
...
int BSTreeContains(const BSTree *root, const ItemType item)
{
 if (root == NULL)
  ...
  return BSTreeContains(root->left, item) || BSTreeContains(root->right, item);
}
