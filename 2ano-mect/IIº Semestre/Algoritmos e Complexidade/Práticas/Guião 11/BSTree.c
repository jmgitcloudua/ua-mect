...
// A: ...Yes . This is the  best order here beacuse the tree start to distroy each
//item, firt on left sub-tree and after on right sub-tree 
// in other words, we are removing nodes without any "children" first which simplifies removal
...
void *BSTreeSearch(const BSTree *header, const void *item)
{
  ...
  while (current != NULL)
  {
    ...
    //<--Solution
    int comp = header->compare(item, current->item);
    if (comp == 0)
    {
      return current->item;
    }
...
// A: ...Yes. This order is required beacuse the tree work correctly
//when we define the the traversal order for each node
