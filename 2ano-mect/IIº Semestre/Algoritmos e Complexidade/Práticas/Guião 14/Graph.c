...
Graph *GraphCopy(const Graph *g)
{
  ...
  ListMoveToHead(vList);
  while (ListGetCurrentPos(vList) > -1)
  {
    struct _Vertex *vertex = (struct _Vertex *)ListGetCurrentItem(vList);
    List *eList = vertex->edgesList;

    ListMoveToHead(eList);
    while (ListGetCurrentPos(eList) > -1)
    {
      struct _Edge *edge = (struct _Edge *)ListGetCurrentItem(eList);
      if (g->isWeighted == 0)
      {
        GraphAddEdge(copy, vertex->id, edge->adjVertex);
      }
      else
      {
        GraphAddWeightedEdge(copy, vertex->id, edge->adjVertex, edge->weight);
      }
      ListMoveToNext(eList); 
    }
    ListMoveToNext(vList);
  }
  return copy;
...
unsigned short GraphRemoveEdge(Graph *g, unsigned int v, unsigned int w)
{
  ...
  if (g->isDigraph == 1)
  {
    ...
    struct _Edge *edge = (struct _Edge *)malloc(sizeof(struct _Edge));
    edge->adjVertex = w;
    if (ListSearch(vertex->edgesList, edge) == 0)
    {
      ListRemoveCurrent(vertex->edgesList);
      vertex->outDegree--; //rows
      ListMove(g->verticesList, w);
      vertex = ListGetCurrentItem(g->verticesList);
      vertex->inDegree--; //column
      g->numEdges--;
    }
    else
    {
