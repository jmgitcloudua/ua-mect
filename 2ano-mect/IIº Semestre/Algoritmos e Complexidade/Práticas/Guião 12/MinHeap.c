...
void MinHeapInsert(MinHeap* ph, void* item) {
  ...
  while (n > 0) {
    ...
    if (ph->compare(item, ph->array[p]) >= 0) break; //<--Solution

    // otherwise, move up the item at node p to open up space for new item
    ph->array[n] = ph->array[p]; //<--Solution

    // update
    n = p ;// the new vacant spot <--Solution
  }
...
void MinHeapRemoveMin(MinHeap* ph) {
  ...
  while (1) {
    ...
    int min = _child(n,1);  // <--Solution    first _child (might not exist)

    if (!(min < ph->size)) break;  // if no second _child, stop looking

    // if second _child is smaller, choose it
    if (ph->compare(ph->array[min], ph->array[min+1]) > 0) { // <--Solution 
       min = min+1;  // <--Solution 
    }
    ...
    ph->array[n] = ph->array[min]; // <--Solution 

    n = min;  //<--Solution  now, the smallest _child spot was just emptied!
  }
  // move last element to emptied spot
  ph->array[n] = ph->array[ph->size]; // <--Solution 

  // mark last element as vacant
  ph->array[ph->size] = NULL; // <--Solution 
}
...
int MinHeapCheck(MinHeap* ph) {
  ...
  for (int n = 1; n < ph->size; n++) {
    int p = _parent(n);
    if (ph->compare(ph->array[n], ph->array[p]) < 0) return 0; // <--Solution
  }
  return 1;
}
