...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < numFiles; i++) {
    ...
    FileReader* fr = FileReaderOpen(argv[i+1]); //<--Solution
  ...
  while (numFiles > 0) {
    ...
    fr = (FileReader *)MinHeapGetMin(H); //<--Solution
    MinHeapRemoveMin(H);//-->
    ...
    int ok = FileReaderNextLine(fr);//<--Solution
