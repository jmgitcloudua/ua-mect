...
public class StrLangMain {
  ...
  public static void main(String[] args) throws IOException {
    for (String fname : args) {
      file = new File(fname);
      if (!file.exists()) {
        System.err.println("ERROR: File" + fname + " is not exist");
        System.exit(2);
      }
      if (file.isDirectory()) {
        System.err.println("ERROR: File" + fname + " ist directory");
        System.exit(2);
      }
      if (!file.canRead()) {
        System.err.println("ERROR: File" + fname + " can not read");
        System.exit(2);
      }
    }
    try {
      ...
      while (lineText != null) {
        ...
        if (sc.hasNextLine()) lineText = sc.nextLine(); else lineText = null;
        numLine++;
      }
