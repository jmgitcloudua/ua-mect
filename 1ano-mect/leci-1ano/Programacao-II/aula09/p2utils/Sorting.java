...
/**
 * This class contains several sorting algorithms and auxiliary methods and
 * fields for evaluating their complexity.
 */
public class Sorting {
  ...
  /**
   * Sequential sort ("greedy" variation of selection sort). Increasing sorting of
   * integer subarray a[start..end[
   * 
   * @param a     the array to be (partially) sorted.
   * @param start index of the first element to sort.
   * @param end   index of the first element not to be sorted (the last element to
   *              sort is {@code end-1}). Requires: a!=null and 0 <= start <= end
   *              <= a.length Ensures: The elements a[k] with start <= k < end are
   *              sorted. the remaining elements are not changed.
   */
   ...
   * @param j index of the other element to swap {@code i},{@code j} must be valid
   *          indexes within array {@code a}
   */
  ...
  public static <E extends Comparable<E>> void mergeSort(E[] a, int start, int end) {
    assert validSubarrayE(a, start, end);
    if (end - start > 1) {
      int middle = (start + end) / 2;
      mergeSort(a, start, middle);
      mergeSort(a, middle, end);
  ...
  public static <E extends Comparable<E>> boolean validSubarray(E[] a, int start, int end) {
    return a != null && 0 <= start && start <= end && end <= a.size();
  }
