...
public class FractionCalculator {
  ...
  public static void main(String[] args) {
    ...
    parts = frac1.split("/");   // divide pela barra
    if (parts.length == 2) { // se tinha barra, extrai numerador e denominador
      x_num = Integer.parseInt(parts[0]);
      x_den = Integer.parseInt(parts[1]);
    }
    ...
    parts = frac2.split("/");   // divide pela barra
    if (parts.length == 2) { // se tinha barra, extrai numerador e denominador
      y_num = Integer.parseInt(parts[0]);
      y_den = Integer.parseInt(parts[1]);
    }
    else {      // senão, extrai numerador e deixa o denominador a 1
      y_num = Integer.parseInt(parts[0]);
      y_den = 1;
