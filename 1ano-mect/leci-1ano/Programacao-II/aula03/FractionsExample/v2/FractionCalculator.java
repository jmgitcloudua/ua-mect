...
public class FractionCalculator {
  ...
  public static void main(String[] args) {
    if (args.length != 3) {
      err.println("Uso: java v2.FractionCalculator <frac1> <op> <frac2>");
      exit(1);
    }
  ...
  static Fraction fractionFromString(String str) {
    ...
    String[] parts = str.split("/");   // divide pela barra
    if (parts.length == 2) { // se tinha barra, extrai numerador e denominador
      f.num = Integer.parseInt(parts[0]);
      f.den = Integer.parseInt(parts[1]);
