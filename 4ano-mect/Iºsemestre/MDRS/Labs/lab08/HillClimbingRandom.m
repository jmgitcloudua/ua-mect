function [CurrentObjective,GlobalBest, GlobalBestSolution] = HillClimbingRandom(nNodes,Links,T,sP,nSP, currentSolution)
...
GlobalBest = Inf;
for iter=1:nFlows
     CurrentSolution2 = RandomStrategy(nFlows,Links,T,sP, nSP ,currentSolution);
     CurrentObjective = HillClimbing(nNodes,Links,T,sP,nSP,CurrentSolution2);
     repeat = true;
     while repeat
         NeighbourBest = Inf;
         for i=1:size(currentSolution,1)
             NeighbourSolution = BuildNeighbours(Links, nNodes, T, bestSol, sP, nSP, currentSolution,i);
             NeighbourObjective = HillClimbing(nNodes,Links,T,sP,nSP,NeighbourSolution);
