...
%  n      - number of adidional VoIP packets flows
%  b      - Bit Error Rate
% OUTPUT PARAMETERS:
    ...
    switch Event
        case ARRIVAL                     % If first event is an ARRIVAL
            ...
            elseif type == 1 % VoIP packet
                ...
                tmp= Clock + unifrnd(0.016,0.024); % 16msec, 24msec
                EventList = [EventList; ARRIVAL, tmp, randi([110,130]), tmp, 1]; % 110 bytes, 130 bytes
                if STATE==0
                    STATE= 1;
        ...
        case DEPARTURE                     % If first event is a DEPARTURE
            if type == 0 % Data Packet
                % Simulate a transmission with a failure (random)
                PBER = (1-b)^(PacketSize*8);%-->
            if rand() < PBER % If success 
