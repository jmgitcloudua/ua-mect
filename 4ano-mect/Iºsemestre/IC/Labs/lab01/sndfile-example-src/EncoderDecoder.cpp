...
EncoderDecoder::EncoderDecoder(const char *filename, char mode, int mvalue)
{
    if (mode != 'd' && mode != 'e')
    {
...
int EncoderDecoder::encode(int n)
{
    n = fold(n);
...
void EncoderDecoder::encodeM(int m)
{
...
void EncoderDecoder::setM(int mi)
{
    this->m = mi;
    b = ceil(log2(m));
}

int EncoderDecoder::decode()
{
    int A = 0;
    int R = 0;
    // int size = 0;
...
void EncoderDecoder::encondeShamt(int shamt)
{
    // Shamt -> 5 bits;
...
int EncoderDecoder::decodeShamt()
{
    char shamt[5];
...
void EncoderDecoder::encodeMode(int mode)
{
    char x = (mode == 0) ? '0' : '1';
    Gfile.writeBit(x);
}

int EncoderDecoder::decodeMode()
{
    return (int)(Gfile.readBit() & 0x01);
}

void EncoderDecoder::encodeHeaderSound(int nFrames, int sampleRate, int Channels, int format, bool lossy)
{
    /*
     * Header
     * Golomb m                 -> 32 bits
...
void EncoderDecoder::decodeHeaderSound(int arr[])
{
...
int EncoderDecoder::unfold(int n)
{
    if (n % 2 == 0)
        return n / 2;
    else
