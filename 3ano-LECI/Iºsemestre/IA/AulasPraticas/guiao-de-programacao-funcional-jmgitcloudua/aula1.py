...
# dgomes@ua.pt -> Prof aulas praticas
...
def capicua(lista):
    if lista == []:
        return True
    return lista[0] == lista[len(lista) - 1] and capicua(lista[1:-1])
...
def juntar(l1, l2):
    if len(l1) != len(l2):
        ...
        return []
    res = juntar(l1[0: len(l1) - 1], l2[0: len(l2) - 1])
    res.append((l1[len(l1) - 1], l2[len(l2) - 1]))
    return res
...
def menor(lista):
    if lista == []:
        return None
    minElem = menor(lista[0: len(lista) - 1])
    if minElem == None:
        minElem = lista[0]
    if minElem > lista[len(lista) - 1]:
        minElem = lista[len(lista) - 1]
    return minElem
