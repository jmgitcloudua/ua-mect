...
typedef struct
{
    ...
    pthread_t *dthr; //douctor threads
    pthread_t *pthr; // patient threads
    uint32_t *pargs; //patient threads's args
...
void init_simulation(uint32_t np, uint32_t nn, uint32_t nd)
{
   ...
   // waiting for all patient to finish
   for (uint32_t i = 0; i < np; i++)
   {
   ...
   //awake blocked thread to terminate safely
   done_pfifo(&hd->triage_queue);
   done_pfifo(&hd->doctor_queue);

     // waiting for nurse to finish
   for (uint32_t i = 0; i < nn; i++)
   {
...
void patient_wait_end_of_consultation(int id)
{
   ...
   //if patient consultation not done, sleep
...
int main(int argc, char *argv[])
{
   ...
   printf("\nSimulation has terminated successfully\n\n");
