...
typedef struct
{
   ...
   pthread_mutex_t acessCR; ///< locking flag wich warrants for mutual exclusion inside monitor
   pthread_cond_t fifoNotFull; /// < condition var to check fifo's fulness
   pthread_cond_t fifoNotEmpty; /// < condition var to check fifo's emptyness
   uint32_t done = 0;
