...
typedef struct
{
    ...
    uint32_t  *pargs; //patient threads args
} HospitalData;
...
void init_simulation(uint32_t np, uint32_t nn, uint32_t nd)
{
   ...
   //Launch doctor threads
   printf("\n");
   printf("Launch %d doctor thread\n",nd);
   for (uint32_t i = 0; i < nd; i++)
   {
      printf("\033[1;35m Doctor %d launched \n\033[0m", i);
      thread_create(&hd->dthr[i], NULL, doctorThread, NULL);
   }
    //Launch nurse threads
   printf("\n");
   printf("Launch %d nurse thread\n",nn);
   for (uint32_t i = 0; i < nn; i++)
   {
      printf("\033[1;36m Nurse %d launched \n\033[0m", i);
      thread_create(&hd->nthr[i], NULL, nurseThread, NULL);
   }
    //Launch patient threads
      printf("\n");
   printf("\nLaunch %d patient thread\n",np);
   for (uint32_t i = 0; i < np; i++)
   {
      printf("\033[1;33m Patient %d launched \n\033[0m", i);
      hd->pargs[i] = i;
      thread_create(&hd->pthr[i], NULL, patientThread, &hd->pargs[i]);
   }
      printf("\n");
   //waiting all patient thread to finish 
   for (uint32_t i = 0; i < np; i++)
   {
      thread_join(hd->pthr[i], NULL);
      printf("Patient %d has terminated\n", i);
   }

   //awake all blocked thread to finish safely
   done_pfifo(&hd->doctor_queue);
   done_pfifo(&hd->triage_queue);

      //waiting all nurse thread to finish 
   for (uint32_t i = 0; i < nn; i++)
   {
      thread_join(hd->nthr[i], NULL);
      printf("Nurse %d has terminated\n", i);
   }
      //waiting all doctor thread to finish 
   for (uint32_t i = 0; i < nd; i++)
   {
      thread_join(hd->dthr[i], NULL);
      printf("Doctor %d has terminated\n", i);
   }
...
int nurse_iteration()//change void to int
{
...
int doctor_iteration()//change void to int
{
...
int main(int argc, char *argv[])
{
   ...
   printf("\n Simulation has terminated successfuly \n");
