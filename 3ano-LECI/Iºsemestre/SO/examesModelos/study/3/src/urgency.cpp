...
typedef struct
{
    ...
    uint32_t *pargs; //patient args threads
...
void init_simulation(uint32_t np, uint32_t nn, uint32_t nd)
{
   ...
   for (uint32_t i = 0; i < nn; i++)
   {
      ...
      //Launch patient thread
   printf("Launch %d patient thread\n", np);
   ...
   //wait for all patient thread to finish
   for (uint32_t i = 0; i < np; i++)
   {
